...
<div class="note">
Normally you'd put markup in a `c:template` declaration, but since the `MailMessage` class expects the mail body as a `string` we use `c:function` because it can return values and can be called directly from C# code.
</div>
